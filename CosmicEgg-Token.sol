...
contract CosmicEgg is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    string private _name = "Cosmic Egg";
    string private _symbol = "COSMIC";
    ...
    // ability for owner to manually swap and liquidfy all the remaining fee collected in contract
    // mainly to save some gas for users in some extreme cases
    function manualSwapAndLiquidfy() public onlyOwner
    {
        uint256 contractTokenBalance = balanceOf(address(this));
        if(contractTokenBalance >= _maxTxAmount)
        {
